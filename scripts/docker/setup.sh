...
EXTRA_MOUNTS="${ICLAW_EXTRA_MOUNTS:-}"
HOME_VOLUME_NAME="${ICLAW_HOME_VOLUME:-}"
RAW_SANDBOX_SETTING="${ICLAW_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${ICLAW_DOCKER_SOCKET:-}"
TIMEZONE="${ICLAW_TZ:-}"
...
read_config_gateway_token() {
  local config_path="$ICLAW_CONFIG_DIR/openclaw.json"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == ICLAW_GATEWAY_TOKEN=* ]]; then
      token="${line#ICLAW_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
sync_gateway_config() {
  ...
  if [[ "${ICLAW_GATEWAY_BIND}" != "loopback" ]]; then
    allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$ICLAW_GATEWAY_PORT" "$ICLAW_GATEWAY_PORT")"
  ...
  batch_json="$(printf '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"%s"}' "$ICLAW_GATEWAY_BIND")"
  ...
  echo "Pinned gateway.mode=local and gateway.bind=$ICLAW_GATEWAY_BIND for Docker setup."
  if [[ -n "$allowed_origin_json" ]]; then
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "ICLAW_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "ICLAW_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
ICLAW_CONFIG_DIR="${ICLAW_CONFIG_DIR:-$HOME/.openclaw}"
ICLAW_WORKSPACE_DIR="${ICLAW_WORKSPACE_DIR:-$HOME/.openclaw/workspace}"

validate_mount_path_value "ICLAW_CONFIG_DIR" "$ICLAW_CONFIG_DIR"
validate_mount_path_value "ICLAW_WORKSPACE_DIR" "$ICLAW_WORKSPACE_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "ICLAW_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "ICLAW_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "ICLAW_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "ICLAW_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "ICLAW_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
  if ! is_valid_timezone "$TIMEZONE"; then
    fail "ICLAW_TZ must match a timezone in /usr/share/zoneinfo (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$ICLAW_CONFIG_DIR"
mkdir -p "$ICLAW_WORKSPACE_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$ICLAW_CONFIG_DIR/identity"
mkdir -p "$ICLAW_CONFIG_DIR/agents/main/agent"
mkdir -p "$ICLAW_CONFIG_DIR/agents/main/sessions"

export ICLAW_CONFIG_DIR
export ICLAW_WORKSPACE_DIR
export ICLAW_GATEWAY_PORT="${ICLAW_GATEWAY_PORT:-18789}"
export ICLAW_BRIDGE_PORT="${ICLAW_BRIDGE_PORT:-18790}"
export ICLAW_GATEWAY_BIND="${ICLAW_GATEWAY_BIND:-lan}"
export ICLAW_IMAGE="$IMAGE_NAME"
export ICLAW_DOCKER_APT_PACKAGES="${ICLAW_DOCKER_APT_PACKAGES:-}"
export ICLAW_EXTENSIONS="${ICLAW_EXTENSIONS:-}"
export ICLAW_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export ICLAW_HOME_VOLUME="$HOME_VOLUME_NAME"
export ICLAW_ALLOW_INSECURE_PRIVATE_WS="${ICLAW_ALLOW_INSECURE_PRIVATE_WS:-}"
export ICLAW_SANDBOX="$SANDBOX_ENABLED"
export ICLAW_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export ICLAW_TZ="$TIMEZONE"
...
if [[ -z "${ICLAW_GATEWAY_TOKEN:-}" ]]; then
  EXISTING_CONFIG_TOKEN="$(read_config_gateway_token || true)"
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    ICLAW_GATEWAY_TOKEN="$EXISTING_CONFIG_TOKEN"
    echo "Reusing gateway token from $ICLAW_CONFIG_DIR/openclaw.json"
  else
    DOTENV_GATEWAY_TOKEN="$(read_env_gateway_token "$ROOT_DIR/.env" || true)"
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      ICLAW_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
      echo "Reusing gateway token from $ROOT_DIR/.env"
    elif command -v openssl >/dev/null 2>&1; then
      ICLAW_GATEWAY_TOKEN="$(openssl rand -hex 32)"
    else
      ICLAW_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${ICLAW_CONFIG_DIR}:/home/node/.openclaw"
    gateway_workspace_mount="${ICLAW_WORKSPACE_DIR}:/home/node/.openclaw/workspace"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${ICLAW_INSTALL_DOCKER_CLI:-}" ]]; then
    export ICLAW_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  ...
  ICLAW_EXTRA_MOUNTS \
  ICLAW_HOME_VOLUME \
  ICLAW_DOCKER_APT_PACKAGES \
  ICLAW_EXTENSIONS \
  ICLAW_SANDBOX \
  ICLAW_DOCKER_SOCKET \
  DOCKER_GID \
  ICLAW_INSTALL_DOCKER_CLI \
  ICLAW_ALLOW_INSECURE_PRIVATE_WS \
  ICLAW_TZ

if [[ "$IMAGE_NAME" == "openclaw:local" ]]; then
  echo "==> Building Docker image: $IMAGE_NAME"
  run_docker_build \
    --build-arg "ICLAW_DOCKER_APT_PACKAGES=${ICLAW_DOCKER_APT_PACKAGES}" \
    --build-arg "ICLAW_EXTENSIONS=${ICLAW_EXTENSIONS}" \
    --build-arg "ICLAW_INSTALL_DOCKER_CLI=${ICLAW_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
echo "Gateway runtime bind comes from ICLAW_GATEWAY_BIND (default: lan)."
echo "Current runtime bind: $ICLAW_GATEWAY_BIND"
echo "Gateway token: $ICLAW_GATEWAY_TOKEN"
...
# --- Sandbox setup (opt-in via ICLAW_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm --entrypoint docker openclaw-gateway --version >/dev/null 2>&1; then
    ...
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg ICLAW_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (ICLAW_IMAGE=openclaw:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
YAML
    ...
    echo "WARNING: ICLAW_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
...
echo "Config: $ICLAW_CONFIG_DIR"
echo "Workspace: $ICLAW_WORKSPACE_DIR"
echo "Token: $ICLAW_GATEWAY_TOKEN"
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f openclaw-gateway"
echo "  ${COMPOSE_HINT} exec openclaw-gateway node dist/index.js health --token \"$ICLAW_GATEWAY_TOKEN\""
