...
ICLAW_AUTH="$HOME/.openclaw/agents/main/agent/auth-profiles.json"
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    if [ "$USE_JSON" -eq 1 ]; then
        ...
        openclaw_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$ICLAW_AUTH" 2>/dev/null || echo "0")
    fi
...
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$ICLAW_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$ICLAW_AUTH" 2>/dev/null || echo "0")
