...
docker_e2e_resolve_image() {
  ...
  if [ -n "${ICLAW_DOCKER_E2E_IMAGE:-}" ]; then
    printf '%s\n' "$ICLAW_DOCKER_E2E_IMAGE"
    return 0
  fi
...
docker_e2e_build_or_reuse() {
  ...
  if [ "${ICLAW_SKIP_DOCKER_BUILD:-0}" = "1" ] || [ "$skip_build" = "1" ]; then
    ...
    if ! docker image inspect "$image_name" >/dev/null 2>&1; then
      ...
      if ! docker pull "$image_name"; then
        ...
        echo "Build it first or unset ICLAW_SKIP_DOCKER_BUILD." >&2
        return 1
      fi
    fi
    return 0
