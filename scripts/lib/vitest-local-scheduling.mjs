...
function isSystemThrottleDisabled(env) {
  const normalized = env.ICLAW_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.ICLAW_VITEST_MAX_WORKERS ?? env.ICLAW_TEST_WORKERS);
  if (override !== null) {
