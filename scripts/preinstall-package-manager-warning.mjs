...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[iclaw] warning: detected ${packageManager} for install lifecycle.`,
    "[iclaw] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[iclaw] prefer: corepack pnpm install",
  ].join("\n");
}
