...
WAIT_FOR_LOCK=0
LOG_PATH="${ICLAW_RESTART_LOG:-/tmp/openclaw-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${ICLAW_GATEWAY_WAIT_SECONDS:-0}"
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  ICLAW_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node iclaw.mjs daemon install --force --runtime node"
      log "  node iclaw.mjs daemon restart"
      log ""
...
choose_app_bundle() {
  ...
  fail "App bundle not found. Set ICLAW_APP_BUNDLE to your installed OpenClaw.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node iclaw.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node iclaw.mjs daemon restart"
