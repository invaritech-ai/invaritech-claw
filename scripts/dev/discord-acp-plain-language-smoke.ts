...
function resolveStateDir(): string {
  const override = process.env.ICLAW_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.ICLAW_HOME?.trim() || process.env.HOME || "";
...
function usage(): string {
  return (
    ...
    "  ICLAW_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  ICLAW_DISCORD_SMOKE_DRIVER\n" +
    "  ICLAW_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  ICLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  ICLAW_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  ICLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  ICLAW_DISCORD_SMOKE_AGENT\n" +
    "  ICLAW_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  ICLAW_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  ICLAW_DISCORD_SMOKE_POLL_MS\n" +
    "  ICLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  ICLAW_DISCORD_SMOKE_ICLAW_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.ICLAW_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw = resolveArg("--driver") || process.env.ICLAW_DISCORD_SMOKE_DRIVER || "token";
    ...
    normalizedDriverMode === "webhook"
      ...
      : normalizedDriverMode === "openclaw"
        ...
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken = resolveArg("--token") || process.env.ICLAW_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.ICLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.ICLAW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") || process.env.ICLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX || "Bot";
  const targetAgent = resolveArg("--agent") || process.env.ICLAW_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.ICLAW_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.ICLAW_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.ICLAW_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.ICLAW_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.ICLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const openclawBin =
    resolveArg("--openclaw-bin") || process.env.ICLAW_DISCORD_SMOKE_ICLAW_BIN || "openclaw";
