...
const urlRaw = getArg("--url") ?? process.env.ICLAW_GATEWAY_URL;
const token = getArg("--token") ?? process.env.ICLAW_GATEWAY_TOKEN;
const nodeHint = getArg("--node");
const dangerous = hasFlag("--dangerous") || process.env.ICLAW_RUN_DANGEROUS === "1";
...
if (!urlRaw || !token) {
  writeStderrLine(
    "Usage: bun scripts/dev/ios-node-e2e.ts --url <wss://host[:port]> --token <gateway.auth.token> [--node <id|name-substring>] [--dangerous] [--json]\n" +
      "Or set env: ICLAW_GATEWAY_URL / ICLAW_GATEWAY_TOKEN",
  );
  process.exit(1);
}
