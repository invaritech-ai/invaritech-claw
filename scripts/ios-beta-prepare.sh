...
PUSH_RELAY_BASE_URL="${ICLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid ICLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid ICLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid ICLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid ICLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing ICLAW_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(ICLAW_URL_SLASH)$(ICLAW_URL_SLASH)#g'
)"
...
ICLAW_CODE_SIGN_STYLE = Automatic
ICLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
ICLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
ICLAW_APP_BUNDLE_ID = ai.openclaw.client
ICLAW_SHARE_BUNDLE_ID = ai.openclaw.client.share
ICLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.openclaw.client.activitywidget
ICLAW_WATCH_APP_BUNDLE_ID = ai.openclaw.client.watchkitapp
ICLAW_WATCH_EXTENSION_BUNDLE_ID = ai.openclaw.client.watchkitapp.extension
ICLAW_APP_PROFILE =
ICLAW_SHARE_PROFILE =
ICLAW_PUSH_TRANSPORT = relay
ICLAW_PUSH_DISTRIBUTION = official
ICLAW_URL_SLASH = /
ICLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
ICLAW_PUSH_APNS_ENVIRONMENT = production
EOF
