...
const TIMINGS_FILE_ENV_KEY = "ICLAW_TEST_PROJECTS_TIMINGS_PATH";
const TIMINGS_DISABLE_ENV_KEY = "ICLAW_TEST_PROJECTS_TIMINGS";
...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.ICLAW_VITEST_MAX_WORKERS || env.ICLAW_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      ICLAW_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
...
async function main() {
  ...
  const isExplicitParallelMultiConfigRun =
    Boolean(process.env.ICLAW_TEST_PROJECTS_PARALLEL) &&
    runSpecs.length > 1 &&
    !runSpecs.some((spec) => spec.watchMode);
  ...
  if (isParallelShardRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        ...
        !process.env.ICLAW_TEST_PROJECTS_PARALLEL &&
        !process.env.ICLAW_VITEST_MAX_WORKERS &&
        !process.env.ICLAW_TEST_WORKERS &&
