...
const skipPackValidationEnv = "ICLAW_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "iclaw") {
    errors.push(`package.json name must be "iclaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.iclaw !== "iclaw.mjs") {
    errors.push(`package.json bin.iclaw must be "iclaw.mjs"; found "${pkg.bin?.iclaw ?? ""}".`);
  }
