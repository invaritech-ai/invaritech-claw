...
const INCLUDE_FILE_ENV_KEY = "ICLAW_VITEST_INCLUDE_FILE";
const FS_MODULE_CACHE_PATH_ENV_KEY = "ICLAW_VITEST_FS_MODULE_CACHE_PATH";
...
const VITEST_NO_OUTPUT_TIMEOUT_ENV_KEY = "ICLAW_VITEST_NO_OUTPUT_TIMEOUT_MS";
const VITEST_NO_OUTPUT_RETRY_ENV_KEY = "ICLAW_VITEST_NO_OUTPUT_RETRY";
...
export function buildFullSuiteVitestRunPlans(args, cwd = process.cwd()) {
  ...
  const parallelShardCount = Number.parseInt(process.env.ICLAW_TEST_PROJECTS_PARALLEL ?? "", 10);
...
export function shouldUseLocalFullSuiteParallelByDefault(env = process.env) {
  ...
  return (
    env.ICLAW_TEST_PROJECTS_SERIAL !== "1" && env.CI !== "true" && env.GITHUB_ACTIONS !== "true"
  );
}
...
function hasConservativeVitestWorkerBudget(env) {
  const workerBudget = parsePositiveInt(env.ICLAW_VITEST_MAX_WORKERS ?? env.ICLAW_TEST_WORKERS);
...
export function resolveParallelFullSuiteConcurrency(specCount, env = process.env, hostInfo) {
  const override = parsePositiveInt(env.ICLAW_TEST_PROJECTS_PARALLEL);
  if (override !== null) {
    return Math.min(override, specCount);
  }
  if (env.ICLAW_TEST_PROJECTS_SERIAL === "1") {
    return 1;
  }
  if (isCiLikeEnv(env)) {
    return 1;
  ...
  if (
    env.ICLAW_TEST_PROJECTS_LEAF_SHARDS !== "1" &&
...
export function shouldAcquireLocalHeavyCheckLock(runSpecs, env = process.env) {
  if (env.ICLAW_TEST_PROJECTS_FORCE_LOCK === "1") {
    return true;
  }
