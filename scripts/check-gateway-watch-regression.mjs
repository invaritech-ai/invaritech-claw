...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  ICLAW_SKIP_ACPX_RUNTIME: "1",
  ICLAW_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$ICLAW_WATCH_PID_FILE"',
    'mkdir -p "$ICLAW_HOME/.openclaw"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$ICLAW_HOME/.openclaw/openclaw.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    ICLAW_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    ICLAW_HOME: isolatedHomeDir,
