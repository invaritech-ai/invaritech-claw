...
function resolveCredentialSource(env: NodeJS.ProcessEnv) {
  return env.ICLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE ?? env.ICLAW_QA_CREDENTIAL_SOURCE;
}

function resolveCredentialRole(env: NodeJS.ProcessEnv) {
  return env.ICLAW_NPM_TELEGRAM_CREDENTIAL_ROLE ?? env.ICLAW_QA_CREDENTIAL_ROLE;
}

async function resolveTrustedOpenClawCommand(rawCommand: string) {
  if (!path.isAbsolute(rawCommand)) {
    throw new Error("ICLAW_NPM_TELEGRAM_SUT_COMMAND must be an absolute path.");
  }
  const commandName = path.basename(rawCommand);
  if (commandName !== "openclaw" && commandName !== "openclaw.cmd") {
    throw new Error(`ICLAW_NPM_TELEGRAM_SUT_COMMAND must point to openclaw; got: ${commandName}`);
  }
  ...
  if (realCommand !== realPrefix && !realCommand.startsWith(`${realPrefix}${path.sep}`)) {
    throw new Error("ICLAW_NPM_TELEGRAM_SUT_COMMAND must resolve inside NPM_CONFIG_PREFIX.");
  }
  return rawCommand;
}

async function main() {
  const rawSutOpenClawCommand = process.env.ICLAW_NPM_TELEGRAM_SUT_COMMAND?.trim();
  if (!rawSutOpenClawCommand) {
    throw new Error("Missing ICLAW_NPM_TELEGRAM_SUT_COMMAND.");
  }
  const sutOpenClawCommand = await resolveTrustedOpenClawCommand(rawSutOpenClawCommand);

  const repoRoot = path.resolve(process.env.ICLAW_NPM_TELEGRAM_REPO_ROOT ?? process.cwd());
  const outputDir =
    process.env.ICLAW_NPM_TELEGRAM_OUTPUT_DIR?.trim() ||
  ...
  const result = await runTelegramQaLive({
    ...
    providerMode: process.env.ICLAW_NPM_TELEGRAM_PROVIDER_MODE,
    primaryModel: process.env.ICLAW_NPM_TELEGRAM_MODEL,
    alternateModel: process.env.ICLAW_NPM_TELEGRAM_ALT_MODEL,
    fastMode: parseBoolean(process.env.ICLAW_NPM_TELEGRAM_FAST),
    scenarioIds: splitCsv(process.env.ICLAW_NPM_TELEGRAM_SCENARIOS),
    sutAccountId: process.env.ICLAW_NPM_TELEGRAM_SUT_ACCOUNT,
  ...
  if (
    !parseBoolean(process.env.ICLAW_NPM_TELEGRAM_ALLOW_FAILURES) &&
