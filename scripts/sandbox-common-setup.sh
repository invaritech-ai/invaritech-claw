...
ICLAW_DOCKER_BUILD_USE_BUILDX="${ICLAW_DOCKER_BUILD_USE_BUILDX:-0}"
ICLAW_DOCKER_BUILD_CACHE_FROM="${ICLAW_DOCKER_BUILD_CACHE_FROM:-}"
ICLAW_DOCKER_BUILD_CACHE_TO="${ICLAW_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${ICLAW_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${ICLAW_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${ICLAW_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${ICLAW_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${ICLAW_DOCKER_BUILD_CACHE_TO}")
  fi
fi
