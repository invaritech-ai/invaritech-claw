...
export const ACPX_ICLAW_TOOLS_MCP_SERVER_NAME = "openclaw-tools";
...
function resolveConfiguredMcpServers(params: {
  ...
  if (params.openClawToolsMcpBridge && resolved[ACPX_ICLAW_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_ICLAW_TOOLS_MCP_SERVER_NAME} is reserved when openClawToolsMcpBridge=true`,
    );
  }
  ...
  if (params.openClawToolsMcpBridge) {
    resolved[ACPX_ICLAW_TOOLS_MCP_SERVER_NAME] = resolveOpenClawToolsMcpServerConfig(
      params.moduleUrl,
    );
  }
  return resolved;
