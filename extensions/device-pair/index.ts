...
function resolveAuthLabel(cfg: OpenClawPluginApi["config"]): ResolveAuthLabelResult {
  ...
  const token =
    pickFirstDefined([process.env.ICLAW_GATEWAY_TOKEN, cfg.gateway?.auth?.token]) ?? undefined;
  const password =
    pickFirstDefined([process.env.ICLAW_GATEWAY_PASSWORD, cfg.gateway?.auth?.password]) ??
    undefined;
