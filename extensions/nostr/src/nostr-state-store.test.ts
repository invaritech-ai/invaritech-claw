...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.ICLAW_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-nostr-"));
  process.env.ICLAW_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.ICLAW_STATE_DIR?.trim();
        if (override) {
          return override;
        }
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.ICLAW_STATE_DIR = previous;
    }
