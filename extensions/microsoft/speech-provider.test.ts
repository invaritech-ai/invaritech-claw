...
describe("listMicrosoftVoices", () => {
  ...
  it("records voice discovery exchanges in debug proxy capture mode", async () => {
    ...
    process.env.ICLAW_DEBUG_PROXY_SESSION_ID = "ms-voices-session";
    ...
    const store = getDebugProxyCaptureStore(
      process.env.ICLAW_DEBUG_PROXY_DB_PATH,
      process.env.ICLAW_DEBUG_PROXY_BLOB_DIR,
    );
    store.upsertSession({
  ...
  it("does not double-capture voice discovery when the global fetch patch is installed", async () => {
    ...
    process.env.ICLAW_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
    process.env.ICLAW_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
    process.env.ICLAW_DEBUG_PROXY_SESSION_ID = "ms-voices-global-session";
