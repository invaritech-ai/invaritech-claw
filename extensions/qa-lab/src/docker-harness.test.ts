...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    expect(compose).toContain("ICLAW_CONFIG_PATH: /tmp/openclaw/openclaw.json");
    expect(compose).toContain("ICLAW_STATE_DIR: /tmp/openclaw/state");
    expect(compose).toContain('ICLAW_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("ICLAW_GATEWAY_TOKEN=qa-token");
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    expect(calls).toEqual([
      expect.stringContaining(
        "docker build -t openclaw:qa-local-prebaked --build-arg ICLAW_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/openclaw",
      ),
    ]);
  });
});
