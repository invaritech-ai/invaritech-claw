...
describe("buildQaRuntimeEnv", () => {
  ...
  it("keeps the slow-reply QA opt-out enabled under fast mode", () => {
    ...
    expect(env.ICLAW_TEST_FAST).toBe("1");
    expect(env.ICLAW_QA_ALLOW_LOCAL_IMAGE_PROVIDER).toBe("1");
    expect(env.ICLAW_ALLOW_SLOW_REPLY_TESTS).toBe("1");
    expect(env.ICLAW_BUNDLED_PLUGINS_DIR).toBe("/tmp/openclaw-qa/bundled-plugins");
    expect(env.ICLAW_COMPATIBILITY_HOST_VERSION).toBe("2026.4.8");
  });

  it("maps live frontier key aliases into provider env vars", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        ICLAW_LIVE_OPENAI_KEY: "openai-live",
        ICLAW_LIVE_ANTHROPIC_KEY: "anthropic-live",
        ICLAW_LIVE_GEMINI_KEY: "gemini-live",
      }),
  ...
  it("preserves Codex CLI auth home for live frontier runs while sandboxing OpenClaw home", async () => {
    ...
    expect(env.ICLAW_HOME).toBe("/tmp/openclaw-qa/home");
  ...
  it("forwards host HOME for live Claude CLI runs while keeping OpenClaw home sandboxed", async () => {
    ...
    expect(env.ICLAW_HOME).toBe("/tmp/openclaw-qa/home");
    expect(env.ICLAW_STATE_DIR).toBe("/tmp/openclaw-qa/state");
  });
  ...
  it("preserves the live Anthropic key for live Claude CLI runs without writing it into config", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        HOME: hostHome,
        ICLAW_LIVE_ANTHROPIC_KEY: "anthropic-live",
        ICLAW_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.ICLAW_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP","ANTHROPIC_API_KEY"]');
    expect(env.ICLAW_LIVE_CLI_BACKEND_AUTH_MODE).toBe("api-key");
  });
  ...
  it("removes preserved Anthropic keys for live Claude CLI subscription runs", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        ...
        ICLAW_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP","ANTHROPIC_API_KEY"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.ICLAW_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP"]');
    expect(env.ICLAW_LIVE_CLI_BACKEND_AUTH_MODE).toBe("subscription");
  });

  it("does not pass QA setup-token values to the gateway child env", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        ICLAW_LIVE_SETUP_TOKEN_VALUE: `sk-ant-oat01-${"a".repeat(80)}`,
        ICLAW_QA_LIVE_ANTHROPIC_SETUP_TOKEN: `sk-ant-oat01-${"b".repeat(80)}`,
      }),
      providerMode: "live-frontier",
    });

    expect(env.ICLAW_LIVE_SETUP_TOKEN_VALUE).toBeUndefined();
    expect(env.ICLAW_QA_LIVE_ANTHROPIC_SETUP_TOKEN).toBeUndefined();
  });

  it("does not pass Convex credential broker secrets to the gateway child env", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        ICLAW_QA_CONVEX_SECRET_CI: "convex-ci-secret",
        ICLAW_QA_CONVEX_SECRET_MAINTAINER: "convex-maintainer-secret",
      }),
      providerMode: "live-frontier",
    });

    expect(env.ICLAW_QA_CONVEX_SECRET_CI).toBeUndefined();
    expect(env.ICLAW_QA_CONVEX_SECRET_MAINTAINER).toBeUndefined();
  });
    ...
    (providerMode) => {
      const env = buildQaRuntimeEnv({
        ...createParams({
          ...
          ICLAW_LIVE_ANTHROPIC_KEY: "anthropic-live",
          ICLAW_LIVE_ANTHROPIC_KEYS: "anthropic-a,anthropic-b",
          ICLAW_LIVE_GEMINI_KEY: "gemini-live",
          ICLAW_LIVE_OPENAI_KEY: "openai-live",
        }),
        providerMode,
      ...
      expect(env.ICLAW_LIVE_OPENAI_KEY).toBeUndefined();
      expect(env.ICLAW_LIVE_ANTHROPIC_KEY).toBeUndefined();
      expect(env.ICLAW_LIVE_ANTHROPIC_KEYS).toBeUndefined();
      expect(env.ICLAW_LIVE_GEMINI_KEY).toBeUndefined();
    },
  );
  ...
  it("stages a live Anthropic setup-token profile for isolated QA workers", async () => {
    ...
    const cfg = await __testing.stageQaLiveAnthropicSetupToken({
      ...
      stateDir,
      env: {
        ICLAW_LIVE_SETUP_TOKEN_VALUE: token,
      },
    });
  ...
  it("preserves only sanitized gateway debug artifacts", async () => {
    ...
    await writeFile(
      stdoutLogPath,
      [
        "ICLAW_GATEWAY_TOKEN=qa-suite-token",
        'OPENAI_API_KEY="openai-live"',
        "ICLAW_QA_CONVEX_SECRET_CI=convex-ci-secret",
        "ICLAW_QA_CONVEX_SECRET_MAINTAINER=convex-maintainer-secret",
    ...
    await expect(readFile(path.join(artifactDir, "gateway.stdout.log"), "utf8")).resolves.toBe(
      [
        "ICLAW_GATEWAY_TOKEN=<redacted>",
        "OPENAI_API_KEY=<redacted>",
        "ICLAW_QA_CONVEX_SECRET_CI=<redacted>",
        "ICLAW_QA_CONVEX_SECRET_MAINTAINER=<redacted>",
        "botToken=<redacted>",
  ...
  it("copies selected live provider configs from the host config", async () => {
    ...
    await expect(
      __testing.readQaLiveProviderConfigOverrides({
        providerIds: ["custom-openai"],
        env: { ICLAW_QA_LIVE_PROVIDER_CONFIG_PATH: configPath },
      }),
    ).resolves.toEqual({
