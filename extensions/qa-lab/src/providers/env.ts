...
const QA_LIVE_ENV_ALIASES = Object.freeze([
  {
    liveVar: "ICLAW_LIVE_OPENAI_KEY",
    providerVar: "OPENAI_API_KEY",
  },
  {
    liveVar: "ICLAW_LIVE_ANTHROPIC_KEY",
    providerVar: "ANTHROPIC_API_KEY",
  },
  {
    liveVar: "ICLAW_LIVE_GEMINI_KEY",
    providerVar: "GEMINI_API_KEY",
  },
]);

export const QA_LIVE_PROVIDER_CONFIG_PATH_ENV = "ICLAW_QA_LIVE_PROVIDER_CONFIG_PATH";
export const QA_LIVE_CLI_BACKEND_PRESERVE_ENV = "ICLAW_LIVE_CLI_BACKEND_PRESERVE_ENV";
export const QA_LIVE_CLI_BACKEND_AUTH_MODE_ENV = "ICLAW_LIVE_CLI_BACKEND_AUTH_MODE";
...
export function resolveQaLiveCliAuthEnv(
  ...
  const hasAnthropicKey = Boolean(
    baseEnv.ANTHROPIC_API_KEY?.trim() || baseEnv.ICLAW_LIVE_ANTHROPIC_KEY?.trim(),
  );
  if (opts?.forwardHostHomeForClaudeCli && authMode === "api-key" && !hasAnthropicKey) {
    throw new Error(
      "Claude CLI API-key QA mode requires ANTHROPIC_API_KEY or ICLAW_LIVE_ANTHROPIC_KEY",
    );
  }
...
export function resolveQaLiveProviderConfigPath(env: NodeJS.ProcessEnv = process.env) {
  const explicit = env[QA_LIVE_PROVIDER_CONFIG_PATH_ENV]?.trim() || env.ICLAW_CONFIG_PATH?.trim();
  return explicit
