...
- Local shell commands receive `ICLAW_SHELL=tui-local` in their environment.
...
- Prefer `openclaw config set` or `openclaw configure` over hand-editing `iclaw.json`.
...
- If your terminal uses a light background and auto-detection is wrong, set `ICLAW_THEME=light` before launching `openclaw tui`.
- To force the original dark palette instead, set `ICLAW_THEME=dark`.
...
- [Config](/cli/config) — inspect, validate, and edit `iclaw.json`
