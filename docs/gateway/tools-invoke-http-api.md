...
- When `gateway.auth.mode="token"`, use `gateway.auth.token` (or `ICLAW_GATEWAY_TOKEN`).
- When `gateway.auth.mode="password"`, use `gateway.auth.password` (or `ICLAW_GATEWAY_PASSWORD`).
