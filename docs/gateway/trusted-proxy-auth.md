...
routes:
  - from: https://openclaw.example.com
    to: http://openclaw-gateway:32768
    policy:
      - allow:
          or:
            - email:
...
location / {
    ...
    proxy_pass http://openclaw:32768;
...
OpenClaw rejects ambiguous configurations where both a `gateway.auth.token` (or `ICLAW_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
