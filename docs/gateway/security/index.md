...
- If someone can modify Gateway host state/config (`~/.iclaw`, including `iclaw.json`), treat them as a trusted operator.
...
- **WhatsApp**: `~/.iclaw/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile` (regular file only; symlinks rejected)
- **Discord bot token**: config/env or SecretRef (env/file/exec providers)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**:
  - `~/.iclaw/credentials/<channel>-allowFrom.json` (default account)
  - `~/.iclaw/credentials/<channel>-<accountId>-allowFrom.json` (non-default accounts)
- **Model auth profiles**: `~/.iclaw/agents/<agentId>/agent/auth-profiles.json`
- **File-backed secrets payload (optional)**: `~/.iclaw/secrets.json`
...
OpenClaw stores session transcripts on disk under `~/.iclaw/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.iclaw` (see the audit section below). If you need
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to the account-scoped pairing allowlist store under `~/.iclaw/credentials/` (`<channel>-allowFrom.json` for default account, `<channel>-<accountId>-allowFrom.json` for non-default accounts), merged with config allowlists.
...
- “Paste the full contents of ~/.iclaw or your logs.”
...
- `~/.iclaw/iclaw.json`: `600` (user read/write only)
- `~/.iclaw`: `700` (user only)
...
Assume anything under `~/.iclaw/` (or `$ICLAW_STATE_DIR/`) may contain secrets or private data:

- `iclaw.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.iclaw`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.iclaw/sandboxes`
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.iclaw/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.iclaw/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.iclaw/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
2. Review the relevant transcript(s): `~/.iclaw/agents/<agentId>/sessions/*.jsonl`.
