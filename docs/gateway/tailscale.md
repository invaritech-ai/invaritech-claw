...
- `token` (default when `ICLAW_GATEWAY_TOKEN` is set)
- `password` (shared secret via `ICLAW_GATEWAY_PASSWORD` or config)
...
Prefer `ICLAW_GATEWAY_PASSWORD` over committing a password to disk.
