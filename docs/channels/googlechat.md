...
4. Store the downloaded JSON file on your gateway host (e.g., `~/.iclaw/googlechat-service-account.json`).
...
2. **Expose the dashboard to the tailnet only (port 8443):**
   ...
   tailscale serve --bg --https 8443 http://127.0.0.1:32768

   # If bound to Tailscale IP only (e.g., 100.106.161.80):
   tailscale serve --bg --https 8443 http://100.106.161.80:32768
   ```
...
3. **Expose only the webhook path publicly:**
   ...
   ```bash
   # If bound to localhost (127.0.0.1 or 0.0.0.0):
   tailscale funnel --bg --set-path /googlechat http://127.0.0.1:32768/googlechat

   # If bound to Tailscale IP only (e.g., 100.106.161.80):
   tailscale funnel --bg --set-path /googlechat http://100.106.161.80:32768/googlechat
   ```
...
```caddy
your-domain.com {
    reverse_proxy /googlechat* localhost:32768
}
```
...
- **Path**: `/googlechat` -> `http://localhost:32768/googlechat`
