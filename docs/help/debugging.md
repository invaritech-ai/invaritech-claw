...
```bash
ICLAW_DEBUG_TIMING=1 pnpm openclaw models list --all --provider moonshot
```
...
```bash
ICLAW_DEBUG_TIMING=json pnpm openclaw models list --all --provider moonshot \
...
```bash
pnpm gateway:dev
ICLAW_PROFILE=dev openclaw tui
```
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`ICLAW_SKIP_CHANNELS=1`).
...
```bash
ICLAW_PROFILE=dev openclaw gateway --dev --reset
```
