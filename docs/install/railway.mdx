...
3. Set the required **Variables** (at least `ICLAW_GATEWAY_PORT` and `ICLAW_GATEWAY_TOKEN`).
4. Enable **HTTP Proxy** on port `8080`.
5. Open `https://<your-railway-domain>/openclaw` and connect using the configured shared secret. This template uses `ICLAW_GATEWAY_TOKEN` by default; if you replace it with password auth, use that password instead.
...
- `ICLAW_GATEWAY_PORT=8080` (required — must match the port in Public Networking)
- `ICLAW_GATEWAY_TOKEN` (required; treat as an admin secret)
- `ICLAW_STATE_DIR=/data/.openclaw` (recommended)
- `ICLAW_WORKSPACE_DIR=/data/workspace` (recommended)
