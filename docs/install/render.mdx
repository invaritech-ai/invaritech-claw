...
services:
  - type: web
    ...
    envVars:
      - key: ICLAW_GATEWAY_PORT
        value: "8080"
      - key: ICLAW_STATE_DIR
        value: /data/.openclaw
      - key: ICLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: ICLAW_GATEWAY_TOKEN
...
`ICLAW_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
- Missing `ICLAW_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `ICLAW_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
