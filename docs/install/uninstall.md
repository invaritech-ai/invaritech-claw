...
```bash
rm -rf "${ICLAW_STATE_DIR:-$HOME/.openclaw}"
```

If you set `ICLAW_CONFIG_PATH` to a custom location outside the state dir, delete that file too.

4. Delete your workspace (optional, removes agent files):

```bash
rm -rf ~/.iclaw/workspace
```
...
- If you used profiles (`--profile` / `ICLAW_PROFILE`), repeat step 3 for each state dir (defaults are `~/.iclaw-<profile>`).
