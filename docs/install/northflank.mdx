...
4. Set the required environment variable: `ICLAW_GATEWAY_TOKEN` (use a strong random value).
...
8. Open the public OpenClaw URL at `/openclaw` and connect using the configured shared secret. This template uses `ICLAW_GATEWAY_TOKEN` by default; if you replace it with password auth, use that password instead.
