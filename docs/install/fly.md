...
<Steps>
  ...
  <Step title="Configure fly.toml">
    ...
    **Key settings:**
    ...
    | `internal_port = 3000`         | Must match `--port 3000` (or `ICLAW_GATEWAY_PORT`) for Fly health checks |
    | `memory = "2048mb"`            | 512MB is too small; 2GB recommended                                         |
    | `ICLAW_STATE_DIR = "/data"` | Persists state on the volume                                                |
  ...
  <Step title="Set secrets">
    ...
    fly secrets set ICLAW_GATEWAY_TOKEN=$(openssl rand -hex 32)
  ...
  <Step title="Access the Gateway">
    ...
    token from `ICLAW_GATEWAY_TOKEN`; if you switched to password auth, use
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `ICLAW_GATEWAY_PORT=3000`).
...
**Fix:** Ensure `ICLAW_STATE_DIR=/data` is set in `fly.toml` and redeploy.
