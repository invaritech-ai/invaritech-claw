...
- Env vars are preferred: `ICLAW_GATEWAY_TOKEN` / `ICLAW_GATEWAY_PASSWORD`.
...
- On macOS, the headless node host executes `system.run` locally by default. Set
  ...
  `ICLAW_NODE_EXEC_FALLBACK=0` to require the app host and fail closed if it is unavailable.
