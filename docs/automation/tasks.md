...
A cron job **definition** lives in `~/.iclaw/cron/jobs.json`; runtime execution state lives beside it in `~/.iclaw/cron/jobs-state.json`. **Every** cron execution creates a task record — both main-session and isolated. Main-session cron tasks default to `silent` notify policy so they track without generating notifications.
