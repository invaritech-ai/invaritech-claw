...
3. **Managed hooks**: `~/.iclaw/hooks/` (user-installed, shared across workspaces). Extra directories from `hooks.internal.load.extraDirs` share this precedence.
...
| command-logger        | `command`                      | Logs all commands to `~/.iclaw/logs/commands.log`     |
...
Logs every slash command to `~/.iclaw/logs/commands.log`.
