...
`~/.iclaw/iclaw.json`. Agent-specific skill visibility lives under
...
- Built-in skill roots always include `~/.iclaw/skills`, `~/.agents/skills`,
...
- Load precedence is `<workspace>/skills` → `<workspace>/.agents/skills` →
  `~/.agents/skills` → `~/.iclaw/skills` → bundled skills →
