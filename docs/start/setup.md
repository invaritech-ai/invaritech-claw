...
- **Tailoring lives outside the repo:** keep your config and workspace in `~/.iclaw/iclaw.json` and `~/.iclaw/workspace/` so repo updates don't touch them.
...
- **Config:** `~/.iclaw/iclaw.json` (JSON/JSON5-ish)
- **Workspace:** `~/.iclaw/workspace` (skills, prompts, memories; make it a private git repo)
...
```bash
node iclaw.mjs gateway --port 32768 --verbose
```
...
- **Wrong port:** Gateway WS defaults to `ws://127.0.0.1:32768`; keep app + CLI on the same port.
- **Where state lives:**
  - Channel/provider state: `~/.iclaw/credentials/`
...
- Keep `~/.iclaw/workspace` and `~/.iclaw/` as “your stuff”; don’t put personal prompts/config into the `openclaw` repo.
