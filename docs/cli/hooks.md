...
Enable a specific hook by adding it to your config (`~/.iclaw/iclaw.json` by default).
...
- Copies the hook pack into `~/.iclaw/hooks/<id>`
...
**Output:** `~/.iclaw/workspace/memory/YYYY-MM-DD-slug.md`
...
**Output:** `~/.iclaw/logs/commands.log`

**View logs:**

```bash
# Recent commands
tail -n 20 ~/.iclaw/logs/commands.log

# Pretty-print
cat ~/.iclaw/logs/commands.log | jq .

# Filter by action
grep '"action":"new"' ~/.iclaw/logs/commands.log | jq .
```
