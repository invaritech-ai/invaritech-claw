...
openclaw secrets reload --url ws://127.0.0.1:32768 --token <token>
```
...
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `iclaw.json` refs)
...
- `configure` targets secret-bearing fields in `iclaw.json` plus `auth-profiles.json` for the selected agent scope.
...
- `iclaw.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.iclaw/.env` known secret keys whose values were migrated
