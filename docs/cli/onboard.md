...
openclaw onboard --mode remote --remote-url wss://gateway-host:32768
```

For plaintext private-network `ws://` targets (trusted networks only), set
`ICLAW_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
There is no `iclaw.json` equivalent for this client-side transport
break-glass.
...
```bash
export ICLAW_GATEWAY_TOKEN="your-token"
openclaw onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env ICLAW_GATEWAY_TOKEN \
  --accept-risk
```
