...
In the active OpenClaw config file (`$ICLAW_CONFIG_PATH`, default `~/.openclaw/openclaw.json`):
...
1. Check server bind: `CONFIG_PATH="${ICLAW_CONFIG_PATH:-${ICLAW_STATE_DIR:-$HOME/.openclaw}/openclaw.json}"; cat "$CONFIG_PATH" | jq '.gateway.bind'`
