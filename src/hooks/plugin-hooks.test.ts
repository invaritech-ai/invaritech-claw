...
describe("bundle plugin hooks", () => {
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.ICLAW_BUNDLED_HOOKS_DIR;
    process.env.ICLAW_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.ICLAW_BUNDLED_HOOKS_DIR;
    } else {
      process.env.ICLAW_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
