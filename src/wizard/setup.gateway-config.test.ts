...
describe("configureGatewayForSetup", () => {
  ...
  it("prefers ICLAW_GATEWAY_TOKEN during quickstart token setup", async () => {
    const prevToken = process.env.ICLAW_GATEWAY_TOKEN;
    process.env.ICLAW_GATEWAY_TOKEN = "token-from-env";
    mocks.randomToken.mockReturnValue("generated-token");
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.ICLAW_GATEWAY_TOKEN = prevToken;
      }
    }
  });
  ...
  it("honors secretInputMode=ref for gateway password prompts", async () => {
    const previous = process.env.ICLAW_GATEWAY_PASSWORD;
    process.env.ICLAW_GATEWAY_PASSWORD = "gateway-secret"; // pragma: allowlist secret
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "password", "off", "env"],
        textQueue: ["18789", "ICLAW_GATEWAY_PASSWORD"],
      });
      const runtime = createRuntime();
      ...
      expect(result.nextConfig.gateway?.auth?.password).toEqual({
        source: "env",
        provider: "default",
        id: "ICLAW_GATEWAY_PASSWORD",
      });
    } finally {
      if (previous === undefined) {
        delete process.env.ICLAW_GATEWAY_PASSWORD;
      } else {
        process.env.ICLAW_GATEWAY_PASSWORD = previous;
      }
    }
  });

  it("stores gateway token as SecretRef when secretInputMode=ref", async () => {
    const previous = process.env.ICLAW_GATEWAY_TOKEN;
    process.env.ICLAW_GATEWAY_TOKEN = "token-from-env";
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "token", "off", "env"],
        textQueue: ["18789", "ICLAW_GATEWAY_TOKEN"],
      });
      const runtime = createRuntime();
      ...
      if (previous === undefined) {
        ...
        process.env.ICLAW_GATEWAY_TOKEN = previous;
      }
    }
  });
