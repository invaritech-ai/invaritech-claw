...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers ICLAW_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for ICLAW_HOME",
      env: { ICLAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when ICLAW_HOME is tilde-only and no fallback home exists",
      env: { ICLAW_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores ICLAW_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          ICLAW_HOME: "/srv/openclaw-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { ICLAW_HOME: "/srv/openclaw-home" } as NodeJS.ProcessEnv,
      },
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of ICLAW_HOME", () => {
    expect(
