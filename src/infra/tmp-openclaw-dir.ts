...
export const POSIX_ICLAW_TMP_DIR = "/tmp/openclaw";
...
export function resolvePreferredOpenClawTmpDir(
  ...
  const existingPreferredState = resolveDirState(POSIX_ICLAW_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_ICLAW_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_ICLAW_TMP_DIR)) {
      return POSIX_ICLAW_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_ICLAW_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_ICLAW_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_ICLAW_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_ICLAW_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_ICLAW_TMP_DIR;
