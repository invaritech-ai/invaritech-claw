...
async function withPresenceModule<T>(
  ...
  return withEnvAsync(
    {
      ICLAW_VERSION: undefined,
      ICLAW_SERVICE_VERSION: undefined,
...
describe("system-presence version fallback", () => {
  ...
  it("uses runtime VERSION when ICLAW_VERSION is not set", async () => {
  ...
  it("prefers ICLAW_VERSION over runtime VERSION", async () => {
    await expectSelfVersion(
      {
  ...
  it("still prefers runtime VERSION over ICLAW_SERVICE_VERSION when ICLAW_VERSION is blank", async () => {
    await expectSelfVersion(
      {
        ICLAW_VERSION: " ",
        ICLAW_SERVICE_VERSION: "2.4.6-service",
  ...
  it("uses runtime VERSION when ICLAW_VERSION and ICLAW_SERVICE_VERSION are blank", async () => {
    await expectSelfVersion(
      {
        ICLAW_VERSION: " ",
        ICLAW_SERVICE_VERSION: "\t",
