...
export const ICLAW_MAIN_PACKAGE_SPEC = "github:openclaw/openclaw#main";
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.ICLAW_UPDATE_PACKAGE_SPEC?.trim() || process.env.ICLAW_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return ICLAW_MAIN_PACKAGE_SPEC;
  }
