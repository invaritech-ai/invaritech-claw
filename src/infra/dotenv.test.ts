...
const CREDENTIAL_AND_GATEWAY_ENV_KEYS = [
  ...
  "ICLAW_LIVE_ANTHROPIC_KEYS",
  "ICLAW_LIVE_GEMINI_KEY",
  "ICLAW_LIVE_OPENAI_KEY",
  "ICLAW_GATEWAY_TOKEN",
  "ICLAW_GATEWAY_PASSWORD",
  "ICLAW_GATEWAY_SECRET",
] as const;

const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "ICLAW_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "ICLAW_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "ICLAW_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "ICLAW_BUNDLED_SKILLS_DIR=./attacker-skills",
...
describe("loadDotEnv", () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.openclaw/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        process.env.ICLAW_STATE_DIR = defaultStateDir;
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ICLAW_STATE_DIR=./evil-state",
            "ICLAW_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.ICLAW_STATE_DIR).toBe(stateDir);
        expect(process.env.ICLAW_CONFIG_PATH).toBeUndefined();
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ICLAW_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "ICLAW_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "ICLAW_LIVE_GEMINI_KEY=sk-gemini-live",
            "ICLAW_LIVE_OPENAI_KEY=sk-openai-live",
            "ICLAW_GATEWAY_TOKEN=attacker-token",
            "ICLAW_GATEWAY_PASSWORD=attacker-password",
            "ICLAW_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
  ...
  it("blocks ICLAW_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
        ...
        expect(process.env.ICLAW_STATE_DIR).toBeUndefined();
        expect(process.env.ICLAW_CONFIG_PATH).toBeUndefined();
      });
    });
  });

  it("blocks path-override vars (ICLAW_AGENT_DIR, ICLAW_BUNDLED_PLUGINS_DIR, PI_CODING_AGENT_DIR, ICLAW_OAUTH_DIR) from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "ICLAW_AGENT_DIR=./evil-agent",
            `ICLAW_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "PI_CODING_AGENT_DIR=./evil-coding",
            "ICLAW_OAUTH_DIR=./evil-oauth",
          ].join("\n"),
        );

        delete process.env.ICLAW_AGENT_DIR;
        delete process.env.ICLAW_BUNDLED_PLUGINS_DIR;
        delete process.env.PI_CODING_AGENT_DIR;
        delete process.env.ICLAW_OAUTH_DIR;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.ICLAW_AGENT_DIR).toBeUndefined();
        expect(process.env.ICLAW_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
        expect(process.env.ICLAW_OAUTH_DIR).toBeUndefined();
      });
    });
  });

  it("blocks ICLAW_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "ICLAW_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.ICLAW_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.ICLAW_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "ICLAW_PINNED_PYTHON=./attacker-python",
            "ICLAW_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.ICLAW_PINNED_PYTHON;
        delete process.env.ICLAW_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.ICLAW_PINNED_PYTHON).toBeUndefined();
        expect(process.env.ICLAW_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ICLAW_PINNED_PYTHON=/trusted/python",
            "ICLAW_PINNED_WRITE_PYTHON=/trusted/write-python",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_BASE_URL;
        ...
        expect(process.env.ICLAW_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.ICLAW_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ICLAW_LIVE_GEMINI_KEY=sk-gemini-live",
            "ICLAW_LIVE_OPENAI_KEY=sk-openai-live",
            "ICLAW_GATEWAY_TOKEN=trusted-token",
            "ICLAW_GATEWAY_PASSWORD=trusted-password",
            "ICLAW_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.ICLAW_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.ICLAW_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.ICLAW_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.ICLAW_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.ICLAW_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.ICLAW_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.ICLAW_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "ICLAW_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  ...
  it("does not load gateway.env when ICLAW_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        process.env.HOME = base;
        process.env.ICLAW_STATE_DIR = customStateDir;
        await writeEnvFile(
          path.join(base, ".config", "openclaw", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.ICLAW_STATE_DIR).toBe(customStateDir);
  ...
  it("keeps the legacy state-dir fallback for CLI dotenv loading", async () => {
    await withIsolatedEnvAndCwd(async () => {
      ...
      process.env.HOME = base;
      delete process.env.ICLAW_STATE_DIR;
      delete process.env.ICLAW_TEST_FAST;
      await fs.mkdir(cwdDir, { recursive: true });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        const runtimeControlKeys = [
          ...
          "ICLAW_WORKSPACE_DIR",
          "ICLAW_MDNS_HOSTNAME",
          "ICLAW_SESSION_CACHE_TTL_MS",
          "ICLAW_UPDATE_PACKAGE_SPEC",
          "ICLAW_GATEWAY_PORT",
          "ICLAW_GATEWAY_URL",
          "ICLAW_CLAWHUB_URL",
          "CLAWHUB_URL",
          ...
          "ICLAW_SKIP_CHANNELS",
          "ICLAW_SKIP_PROVIDERS",
          "ICLAW_SKIP_CRON",
          "ICLAW_RAW_STREAM",
          "ICLAW_RAW_STREAM_PATH",
          "ICLAW_CACHE_TRACE",
          "ICLAW_CACHE_TRACE_FILE",
          "ICLAW_CACHE_TRACE_MESSAGES",
          "ICLAW_CACHE_TRACE_PROMPT",
          "ICLAW_CACHE_TRACE_SYSTEM",
          "ICLAW_SHOW_SECRETS",
          "ICLAW_PLUGIN_CATALOG_PATHS",
          "ICLAW_MPM_CATALOG_PATHS",
          "ICLAW_NODE_EXEC_HOST",
          "ICLAW_NODE_EXEC_FALLBACK",
          "ICLAW_ALLOW_PROJECT_LOCAL_BIN",
