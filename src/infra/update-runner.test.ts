...
describe("runGatewayUpdate", () => {
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(path.join(tempDir, "iclaw.mjs"), "export {};\n", "utf-8");
  });
  ...
  async function createStableTagRunner(params: {
    ...
    const doctorKey = `${doctorNodePath} ${path.join(tempDir, "iclaw.mjs")} doctor --non-interactive --fix`;
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      [`${doctorNodePath} ${path.join(tempDir, "iclaw.mjs")} doctor --non-interactive --fix`]: {
        stdout: "",
      },
    });
  ...
  it("bootstraps pnpm via npm when pnpm and corepack are unavailable", async () => {
    ...
    const { calls, runCommand } = createGitInstallRunner({
      ...
      doctorCommand: `${process.execPath} ${path.join(tempDir, "iclaw.mjs")} doctor --non-interactive`,
  ...
  it("uses npm-bootstrapped pnpm for dev preflight when pnpm and corepack are missing", async () => {
    ...
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(tempDir, "iclaw.mjs")} doctor --non-interactive --fix`;
  ...
  it("falls back to the cloned cwd when git root probing misses a fresh checkout", async () => {
    ...
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(gitRoot, "iclaw.mjs")} doctor --non-interactive --fix`;
  ...
  it("prepends portable Git PATH for global Windows npm updates", async () => {
    ...
    const portableGitUsr = path.join(localAppData, "iclaw", "deps", "portable-git", "usr", "bin");
  ...
  it("uses ICLAW_UPDATE_PACKAGE_SPEC for global package updates", async () => {
    const { nodeModules, pkgRoot } = await createGlobalPackageFixture(tempDir);
    ...
    await withEnvAsync(
      { ICLAW_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/openclaw-next.tgz" },
      async () => {
        const result = await runWithCommand(runCommand, { cwd: pkgRoot });
        expect(result.status).toBe("ok");
      },
  ...
  it("fails with a clear reason when iclaw.mjs is missing", async () => {
    await setupGitCheckout({ packageManager: "pnpm@8.0.0" });
    await fs.rm(path.join(tempDir, "iclaw.mjs"), { force: true });
