...
function expectLaunchdSupervisedWithoutKickstart(params?: { launchJobLabel?: string }) {
  ...
  process.env.ICLAW_LAUNCHD_LABEL = "ai.openclaw.gateway";
...
describe("restartGatewayProcessWithFreshPid", () => {
  it("returns disabled when ICLAW_NO_RESPAWN is set", () => {
    process.env.ICLAW_NO_RESPAWN = "1";
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("disabled");
    expect(spawnMock).not.toHaveBeenCalled();
  });

  it("keeps ICLAW_NO_RESPAWN ahead of inherited supervisor hints", () => {
  ...
  it("spawns detached child with current exec argv", () => {
    delete process.env.ICLAW_NO_RESPAWN;
    clearSupervisorHints();
    setPlatform("linux");
  ...
  it("returns supervised when ICLAW_LAUNCHD_LABEL is set (stock launchd plist)", () => {
    clearSupervisorHints();
    expectLaunchdSupervisedWithoutKickstart();
  });

  it("returns supervised when ICLAW_SYSTEMD_UNIT is set", () => {
    clearSupervisorHints();
    setPlatform("linux");
    process.env.ICLAW_SYSTEMD_UNIT = "openclaw-gateway.service";
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
    expect(spawnMock).not.toHaveBeenCalled();
  });
  ...
  it("returns supervised when OpenClaw gateway task markers are set on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
    process.env.ICLAW_SERVICE_MARKER = "openclaw";
    process.env.ICLAW_SERVICE_KIND = "gateway";
  ...
  it("ignores node task script hints for gateway restart detection on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
    process.env.ICLAW_TASK_SCRIPT = "C:\\openclaw\\node.cmd";
    process.env.ICLAW_TASK_SCRIPT_NAME = "node.cmd";
    process.env.ICLAW_SERVICE_MARKER = "openclaw";
    process.env.ICLAW_SERVICE_KIND = "node";
