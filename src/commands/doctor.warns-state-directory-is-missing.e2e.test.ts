...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    process.env.ICLAW_STATE_DIR = missingDir;
    await doctorCommand(createDoctorRuntime(), {
      nonInteractive: true,
      workspaceSuggestions: false,
    });
  ...
  it("skips gateway auth warning when ICLAW_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    process.env.ICLAW_GATEWAY_TOKEN = "env-token-1234567890";
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
  ...
  it("keeps doctor read-only when gateway token is SecretRef-managed but unresolved", async () => {
    ...
    const previousToken = process.env.ICLAW_GATEWAY_TOKEN;
    delete process.env.ICLAW_GATEWAY_TOKEN;
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
      ...
      if (previousToken === undefined) {
        ...
        process.env.ICLAW_GATEWAY_TOKEN = previousToken;
      }
    }
