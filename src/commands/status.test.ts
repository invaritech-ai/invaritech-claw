...
beforeAll(() => {
  envSnapshot = captureEnv(["ICLAW_PROFILE"]);
  process.env.ICLAW_PROFILE = "isolated";
});
...
async function createMockStatusScanResult(params: { includePluginCompatibility?: boolean } = {}) {
  ...
  return {
    ...
    gatewayProbeAuth: process.env.ICLAW_GATEWAY_TOKEN
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.ICLAW_GATEWAY_TOKEN?.trim();
...
describe("statusCommand", () => {
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("ICLAW_GATEWAY_TOKEN", "abcd1234", async () => {
