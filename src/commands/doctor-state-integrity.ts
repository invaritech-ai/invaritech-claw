...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for ICLAW_STATE_DIR.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // ICLAW_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        `  Set locally: ICLAW_STATE_DIR=~/.openclaw ${formatCliCommand("openclaw doctor")}`,
      ].join("\n"),
    );
  }
