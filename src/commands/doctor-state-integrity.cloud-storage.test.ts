...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("anchors cloud detection to OS homedir when ICLAW_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".openclaw");
    const originalOpenClawHome = process.env.ICLAW_HOME;
    process.env.ICLAW_HOME = "/tmp/openclaw-home-override";
    ...
    try {
      ...
      if (originalOpenClawHome === undefined) {
        delete process.env.ICLAW_HOME;
      } else {
        process.env.ICLAW_HOME = originalOpenClawHome;
      }
    }
  });
