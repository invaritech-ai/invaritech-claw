...
function resolvePairingSetupAuthLabel(
  ...
  const envToken = normalizeOptionalString(env.ICLAW_GATEWAY_TOKEN);
  const envPassword = normalizeOptionalString(env.ICLAW_GATEWAY_PASSWORD);
  const token =
...
export async function resolvePairingSetupFromConfig(
  ...
  const cfgForAuth = await materializeGatewayAuthSecretRefs({
    ...
    hasTokenCandidate: Boolean(normalizeOptionalString(env.ICLAW_GATEWAY_TOKEN)),
    hasPasswordCandidate: Boolean(normalizeOptionalString(env.ICLAW_GATEWAY_PASSWORD)),
  });
