...
import { ICLAW_CLI_ENV_VALUE } from "../infra/openclaw-exec-env.js";
...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    ...
    expect(resolved.ICLAW_BASE_ENV).toBe("base");
    expect(resolved.ICLAW_TEST_ENV).toBe("ok");
    expect(resolved.ICLAW_TO_REMOVE).toBeUndefined();
    expect(resolved.ICLAW_CLI).toBe(ICLAW_CLI_ENV_VALUE);
  });
