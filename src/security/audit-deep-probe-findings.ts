...
export function collectDeepProbeFindings(params: {
  ...
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set ICLAW_GATEWAY_TOKEN/ICLAW_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("openclaw security audit --deep")}".`,
    });
  }
  return findings;
}
