...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds ICLAW_GATEWAY_TOKEN and should be reinstalled.",
  ...
  issues.push({
    ...
    message:
      "Gateway service ICLAW_GATEWAY_TOKEN does not match gateway.auth.token in openclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.ICLAW_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.ICLAW_GATEWAY_TOKEN);
}
