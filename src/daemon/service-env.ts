...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.ICLAW_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    ICLAW_PROFILE: profile,
    ICLAW_GATEWAY_PORT: String(port),
    ICLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    ICLAW_SYSTEMD_UNIT: systemdUnit,
    ICLAW_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    ICLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    ICLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    ICLAW_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.ICLAW_GATEWAY_TOKEN);
  const allowInsecurePrivateWs = normalizeOptionalString(env.ICLAW_ALLOW_INSECURE_PRIVATE_WS);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    ICLAW_GATEWAY_TOKEN: gatewayToken,
    ICLAW_ALLOW_INSECURE_PRIVATE_WS: allowInsecurePrivateWs,
    ICLAW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    ICLAW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    ICLAW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    ICLAW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    ICLAW_LOG_PREFIX: "node",
    ICLAW_SERVICE_MARKER: NODE_SERVICE_MARKER,
    ICLAW_SERVICE_KIND: NODE_SERVICE_KIND,
    ICLAW_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    ICLAW_STATE_DIR: sharedEnv.stateDir,
    ICLAW_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.ICLAW_STATE_DIR;
  const configPath = env.ICLAW_CONFIG_PATH;
