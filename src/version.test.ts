...
const versionFixtureRoot = createSuiteTempRootTracker({ prefix: "iclaw-version-" });
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "iclaw", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "iclaw", version: "2.3.4" });
  ...
  it("ignores legacy openclaw package metadata", async () => {
    await withVersionFixtureDir(async (root) => {
  ...
  it("ignores non-iclaw package and blank build-info versions", async () => {
    await withVersionFixtureDir(async (root) => {
  ...
  it("prefers ICLAW_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        ICLAW_VERSION: "9.9.9",
        ICLAW_SERVICE_VERSION: "2.2.2",
        npm_package_version: "1.1.1",
      }),
    ).toBe("9.9.9");
  });

  it("prefers runtime VERSION over stale ICLAW_VERSION for compatibility checks", () => {
    const previous = process.env.ICLAW_VERSION;
    const previousService = process.env.ICLAW_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.ICLAW_VERSION = "2026.3.25";
      process.env.ICLAW_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      process.env.ICLAW_VERSION = previous;
      process.env.ICLAW_SERVICE_VERSION = previousService;
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        ICLAW_VERSION: "2026.3.99",
        ICLAW_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        ICLAW_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        ICLAW_VERSION: "2026.3.99",
        ICLAW_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
