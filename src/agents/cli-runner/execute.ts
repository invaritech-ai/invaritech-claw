...
const CLI_BACKEND_PRESERVE_ENV = "ICLAW_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.ICLAW_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.ICLAW_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.ICLAW_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.ICLAW_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.ICLAW_MCP_MESSAGE_CHANNEL || "<empty>"}`,
    `senderIsOwner=${childEnv.ICLAW_MCP_SENDER_IS_OWNER || "<empty>"}`,
  ].join(" ");
}
    ...
    return await enqueueCliRun(queueKey, async () => {
      ...
      try {
        ...
        if (logOutputText) {
          ...
          if (env.ICLAW_MCP_TOKEN || env.ICLAW_MCP_SESSION_KEY || env.ICLAW_MCP_SENDER_IS_OWNER) {
