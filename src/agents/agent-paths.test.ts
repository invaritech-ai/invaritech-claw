...
describe("resolveOpenClawAgentDir", () => {
  ...
  it("honors ICLAW_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          ICLAW_STATE_DIR: undefined,
          ICLAW_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveOpenClawAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when ICLAW_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers ICLAW_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          ICLAW_STATE_DIR: undefined,
          ICLAW_AGENT_DIR: primaryOverride,
