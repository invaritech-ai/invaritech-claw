...
const GOOGLE_LIVE_SINGLE_KEY = "ICLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "ICLAW_LIVE_ANTHROPIC_KEY",
    listVar: "ICLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "ICLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `ICLAW_LIVE_${base}_KEY`;
