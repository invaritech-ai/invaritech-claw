...
describe("discoverAuthStorage", () => {
  ...
  it("preserves legacy auth.json when auth store is forced read-only", async () => {
    await withAgentDir(async (agentDir) => {
      const previous = process.env.ICLAW_AUTH_STORE_READONLY;
      process.env.ICLAW_AUTH_STORE_READONLY = "1";
      try {
        await writeLegacyAuthJson(agentDir, {
          openrouter: { type: "api_key", key: "legacy-static-key" },
        });
        ...
        if (previous === undefined) {
          delete process.env.ICLAW_AUTH_STORE_READONLY;
        } else {
          process.env.ICLAW_AUTH_STORE_READONLY = previous;
        }
      }
    });
  });
  ...
  it("includes env-backed provider auth when no auth profile exists", async () => {
    ...
    const previousDisableBundledPlugins = process.env.ICLAW_DISABLE_BUNDLED_PLUGINS;
    process.env.MISTRAL_API_KEY = "mistral-env-test-key";
    delete process.env.ICLAW_BUNDLED_PLUGINS_DIR;
    delete process.env.ICLAW_DISABLE_BUNDLED_PLUGINS;
    try {
      ...
      if (previousDisableBundledPlugins === undefined) {
        ...
        process.env.ICLAW_DISABLE_BUNDLED_PLUGINS = previousDisableBundledPlugins;
      }
    }
  });
});
