...
describe("provider attribution", () => {
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { ICLAW_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
    ...
    expect(resolveProviderAttributionHeaders("openai", { ICLAW_VERSION: "2026.3.22" })).toEqual({
      originator: "openclaw",
      version: "2026.3.22",
      "User-Agent": "openclaw/2026.3.22",
    });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { ICLAW_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ ICLAW_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
