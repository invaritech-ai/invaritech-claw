...
describe("resolveAgentConfig", () => {
  ...
  it("uses ICLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "openclaw-home");
    vi.stubEnv("ICLAW_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as OpenClawConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".openclaw", "workspace"));
  });

  it("uses ICLAW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "openclaw-home");
    vi.stubEnv("ICLAW_HOME", home);
    // Clear state dir so it falls back to ICLAW_HOME
    vi.stubEnv("ICLAW_STATE_DIR", "");
  ...
  it("non-default agent without defaults.workspace falls back to stateDir", () => {
    ...
    vi.stubEnv("ICLAW_STATE_DIR", stateDir);
    const cfg: OpenClawConfig = {
      agents: {
        list: [{ id: "main" }, { id: "work", default: true, workspace: "/work-ws" }],
      },
