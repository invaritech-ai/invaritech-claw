...
export function resolveConfigDir(
  ...
  const configPath = env.ICLAW_CONFIG_PATH?.trim();
  if (configPath) {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.ICLAW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$ICLAW_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via ICLAW_STATE_DIR.
