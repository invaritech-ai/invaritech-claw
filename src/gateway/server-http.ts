...
export async function runGatewayHttpRequestStages(
  ...
  for (const stage of stages) {
    try {
      ...
      // Log and skip the failing stage so subsequent stages
...
export function createGatewayHttpServer(opts: {
  ...
  // clients is unused in HTTP scope after canvas-host removal
  void opts.clients;
  ...
  async function handleRequest(req: IncomingMessage, res: ServerResponse) {
    ...
    try {
      ...
      // Plugin routes run before the catch-all so explicitly
...
export function attachGatewayUpgradeHandler(opts: {
  ...
  const { httpServer, wss, preauthConnectionBudget, resolvedAuth, log } = opts;
  const getResolvedAuth = opts.getResolvedAuth ?? (() => resolvedAuth);
  void getResolvedAuth;
