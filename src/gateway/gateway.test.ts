...
async function setupGatewayTempHome(params: { prefix: string; minimalGateway?: boolean }) {
  const envSnapshot = captureEnv([
    ...
    ...(params.minimalGateway ? (["ICLAW_TEST_MINIMAL_GATEWAY"] as const) : []),
  ]);
  ...
  process.env.ICLAW_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      process.env.ICLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
      process.env.ICLAW_SKIP_PROVIDERS = "1";
      process.env.ICLAW_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
      process.env.ICLAW_TEST_MINIMAL_GATEWAY = "1";
