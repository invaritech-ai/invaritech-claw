...
const ACP_BIND_LIVE = isTruthyEnvValue(process.env.ICLAW_LIVE_ACP_BIND);
...
function resolveLiveParentModel(): string {
  return normalizeOpenAiModelRef(
    process.env.ICLAW_LIVE_ACP_BIND_PARENT_MODEL?.trim() ||
      process.env.ICLAW_LIVE_ACP_BIND_CODEX_MODEL?.trim() ||
...
async function prepareCodexHomeForLiveBindTest(): Promise<void> {
  ...
  const model = process.env.ICLAW_LIVE_ACP_BIND_CODEX_MODEL?.trim() || DEFAULT_LIVE_CODEX_MODEL;
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        port: process.env.ICLAW_GATEWAY_PORT,
      ...
      const liveAgent = normalizeAcpAgent(process.env.ICLAW_LIVE_ACP_BIND_AGENT);
      const agentCommandOverride =
        process.env.ICLAW_LIVE_ACP_BIND_AGENT_COMMAND?.trim() || undefined;
      ...
      try {
        ...
        if (previous.port === undefined) {
          ...
          process.env.ICLAW_GATEWAY_PORT = previous.port;
        }
