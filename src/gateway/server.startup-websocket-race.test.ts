...
describe("gateway startup websocket readiness", () => {
  it("does not bind the websocket port until websocket handlers are attached", async () => {
    ...
    const previousMinimal = process.env.ICLAW_TEST_MINIMAL_GATEWAY;
    process.env.ICLAW_TEST_MINIMAL_GATEWAY = "0";
    ...
    try {
      ...
      if (previousMinimal === undefined) {
        ...
        process.env.ICLAW_TEST_MINIMAL_GATEWAY = previousMinimal;
      }
    }
  });
