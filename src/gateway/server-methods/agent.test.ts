...
const ORIGINAL_STATE_DIR = process.env.ICLAW_STATE_DIR;
...
describe("gateway agent handler", () => {
  afterEach(() => {
    if (ORIGINAL_STATE_DIR === undefined) {
      ...
      process.env.ICLAW_STATE_DIR = ORIGINAL_STATE_DIR;
    }
    resetDetachedTaskLifecycleRuntimeForTests();
  ...
  it("does not create task rows for inter-session completion wakes", async () => {
    ...
    await invokeAgent(
      {
        message: [
          "[Mon 2026-04-06 02:42 GMT+1] <<<BEGIN_ICLAW_INTERNAL_CONTEXT>>>",
