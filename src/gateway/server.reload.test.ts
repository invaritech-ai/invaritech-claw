...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.ICLAW_SKIP_CHANNELS;
    prevSkipGmail = process.env.ICLAW_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.ICLAW_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    process.env.ICLAW_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.ICLAW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.ICLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.ICLAW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.ICLAW_SKIP_PROVIDERS;
    } else {
      process.env.ICLAW_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeConfigFile(config: unknown) {
    ...
    if (!configPath) {
      throw new Error("ICLAW_CONFIG_PATH is not set");
    }
  ...
  async function withNonMinimalGatewayServer(
    ...
    return await withEnvAsync({ ICLAW_TEST_MINIMAL_GATEWAY: undefined }, async () =>
  ...
  it("keeps last-known-good auth snapshot active when gateway auth token exec reload fails", async () => {
    const stateDir = process.env.ICLAW_STATE_DIR;
    if (!stateDir) {
      throw new Error("ICLAW_STATE_DIR is not set");
    }
...
`,
    ...
    const previousGatewayTokenEnv = process.env.ICLAW_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.ICLAW_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
