...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.ICLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.ICLAW_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.ICLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.ICLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });

    test("prefers ICLAW_HANDSHAKE_TIMEOUT_MS and falls back on empty string", () => {
      const prevHandshakeTimeout = process.env.ICLAW_HANDSHAKE_TIMEOUT_MS;
      const prevTestHandshakeTimeout = process.env.ICLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.ICLAW_HANDSHAKE_TIMEOUT_MS = "75";
      process.env.ICLAW_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(75);
        process.env.ICLAW_HANDSHAKE_TIMEOUT_MS = "";
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(20);
      } finally {
        if (prevHandshakeTimeout === undefined) {
          delete process.env.ICLAW_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.ICLAW_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
        if (prevTestHandshakeTimeout === undefined) {
          delete process.env.ICLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.ICLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevTestHandshakeTimeout;
        }
      }
    });
      ...
      for (const testCase of [
        {
          env: {
            ICLAW_VERSION: " ",
            ICLAW_SERVICE_VERSION: "2.4.6-service",
