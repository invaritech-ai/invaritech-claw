...
const ORIGINAL_GATEWAY_TOKEN_ENV = process.env.ICLAW_GATEWAY_TOKEN;
const OLD_TOKEN = "shared-token-old";
const NEW_TOKEN = "shared-token-new";
const DEFERRED_RESTART_DELAY_MS = 1_000;
const SECRET_REF_TOKEN_ID = "ICLAW_SHARED_AUTH_ROTATION_SECRET_REF";
...
afterAll(() => {
  ...
  if (ORIGINAL_GATEWAY_TOKEN_ENV === undefined) {
    ...
    process.env.ICLAW_GATEWAY_TOKEN = ORIGINAL_GATEWAY_TOKEN_ENV;
  }
});
...
describe("gateway shared auth rotation with unchanged SecretRefs", () => {
  ...
  beforeAll(async () => {
    ...
    if (!configPath) {
      throw new Error("ICLAW_CONFIG_PATH missing in gateway test environment");
    }
