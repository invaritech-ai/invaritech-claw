...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.ICLAW_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.ICLAW_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.ICLAW_LIVE_GATEWAY_SMOKE);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.ICLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_MODEL_TIMEOUT_MS = resolveGatewayLiveModelTimeoutMs();
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.ICLAW_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.ICLAW_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["ICLAW_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayRaw = process.env.ICLAW_LIVE_GATEWAY_MAX_MODELS?.trim();
  if (gatewayRaw) {
    return Math.max(0, toInt(gatewayRaw, 0));
  }
  const rawModels = process.env.ICLAW_LIVE_GATEWAY_MODELS?.trim();
  const useExplicitModels = Boolean(rawModels) && rawModels !== "modern" && rawModels !== "all";
  return resolveHighSignalLiveModelLimit({
    rawMaxModels: process.env.ICLAW_LIVE_MAX_MODELS,
    useExplicitModels,
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.ICLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.ICLAW_LIVE_MODEL_TIMEOUT_MS,
...
describe("resolveGatewayLiveMaxModels", () => {
  const originalGatewayModels = process.env.ICLAW_LIVE_GATEWAY_MODELS;
  const originalGatewayMax = process.env.ICLAW_LIVE_GATEWAY_MAX_MODELS;
  const originalSharedMax = process.env.ICLAW_LIVE_MAX_MODELS;
  ...
  afterEach(() => {
    restoreEnvValue("ICLAW_LIVE_GATEWAY_MODELS", originalGatewayModels);
    restoreEnvValue("ICLAW_LIVE_GATEWAY_MAX_MODELS", originalGatewayMax);
    restoreEnvValue("ICLAW_LIVE_MAX_MODELS", originalSharedMax);
  });

  it("defaults modern gateway sweeps to the curated high-signal cap", () => {
    delete process.env.ICLAW_LIVE_GATEWAY_MODELS;
    delete process.env.ICLAW_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.ICLAW_LIVE_MAX_MODELS;
  ...
  it("keeps explicit gateway model lists uncapped unless a cap is provided", () => {
    process.env.ICLAW_LIVE_GATEWAY_MODELS = "openai/gpt-5.5,anthropic/claude-opus-4-6";
    delete process.env.ICLAW_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.ICLAW_LIVE_MAX_MODELS;

    expect(resolveGatewayLiveMaxModels()).toBe(0);

    process.env.ICLAW_LIVE_GATEWAY_MAX_MODELS = "2";
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    ...
    token: process.env.ICLAW_GATEWAY_TOKEN,
    ...
    disableBonjour: process.env.ICLAW_DISABLE_BONJOUR,
    logLevel: process.env.ICLAW_LOG_LEVEL,
    agentDir: process.env.ICLAW_AGENT_DIR,
  ...
  if (QUIET_LIVE_LOGS) {
    process.env.ICLAW_DISABLE_BONJOUR = "1";
    process.env.ICLAW_LOG_LEVEL = "silent";
  }
  ...
  process.env.ICLAW_STATE_DIR = tempStateDir;
  ...
  process.env.ICLAW_AGENT_DIR = tempAgentDir;
  ...
  process.env.ICLAW_CONFIG_PATH = tempConfigPath;
  ...
  try {
    ...
    process.env.ICLAW_CONFIG_PATH = previous.configPath;
    process.env.ICLAW_GATEWAY_TOKEN = previous.token;
    process.env.ICLAW_SKIP_CHANNELS = previous.skipChannels;
    process.env.ICLAW_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.ICLAW_SKIP_CRON = previous.skipCron;
    process.env.ICLAW_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.ICLAW_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.ICLAW_LOG_LEVEL = previous.logLevel;
    process.env.ICLAW_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.ICLAW_STATE_DIR = previous.stateDir;
  }
}
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via ICLAW_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
