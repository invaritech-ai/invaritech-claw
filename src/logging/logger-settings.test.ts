...
beforeEach(() => {
  originalTestFileLog = process.env.ICLAW_TEST_FILE_LOG;
  originalOpenClawLogLevel = process.env.ICLAW_LOG_LEVEL;
  delete process.env.ICLAW_TEST_FILE_LOG;
  delete process.env.ICLAW_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.ICLAW_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalOpenClawLogLevel === undefined) {
    delete process.env.ICLAW_LOG_LEVEL;
  } else {
    process.env.ICLAW_LOG_LEVEL = originalOpenClawLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    process.env.ICLAW_TEST_FILE_LOG = "1";
    const settings = logging.getResolvedLoggerSettings();
