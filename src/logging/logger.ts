...
function resolveDefaultLogDir(): string {
  return canUseNodeFs() ? resolvePreferredOpenClawTmpDir() : POSIX_ICLAW_TMP_DIR;
}

function resolveDefaultLogFile(defaultLogDir: string): string {
  return canUseNodeFs()
    ? path.join(defaultLogDir, "openclaw.log")
    : `${POSIX_ICLAW_TMP_DIR}/openclaw.log`;
}
...
function resolveSettings(): ResolvedSettings {
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.ICLAW_TEST_FILE_LOG !== "1" ? "silent" : "info";
