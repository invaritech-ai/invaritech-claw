...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "ICLAW_ENV_TEST_A";
    const keyB = "ICLAW_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "ICLAW_ENV_TEST_ADDED";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "ICLAW_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "ICLAW_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "ICLAW_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "ICLAW_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "ICLAW_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    ...
    const previousOpenClawHome = process.env.ICLAW_HOME;
    const previousStateDir = process.env.ICLAW_STATE_DIR;
    const previousBundledDir = process.env.ICLAW_BUNDLED_PLUGINS_DIR;
    process.env.ICLAW_HOME = "/srv/openclaw-home";
    process.env.ICLAW_STATE_DIR = "/srv/openclaw-state";
    process.env.ICLAW_BUNDLED_PLUGINS_DIR = "/srv/openclaw-bundled";
    ...
    try {
      ...
      expect(env.ICLAW_HOME).toBeUndefined();
      expect(env.ICLAW_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.ICLAW_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("ICLAW_HOME", previousOpenClawHome);
      restoreEnvKey("ICLAW_STATE_DIR", previousStateDir);
      restoreEnvKey("ICLAW_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
    ...
    try {
      const seen = withPathResolutionEnv(
        ...
        (env) => ({
          ...
          processOpenClawHome: process.env.ICLAW_HOME,
          processBundledDir: process.env.ICLAW_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.ICLAW_BUNDLED_PLUGINS_DIR,
        }),
      );
      ...
      expect(process.env.ICLAW_HOME).toBe("/srv/openclaw-home");
    } finally {
      restoreEnvKey("ICLAW_HOME", previousOpenClawHome);
    }
  });
