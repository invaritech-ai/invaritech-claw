...
async function withExecDryRunConfigHarness(
  ...
  const envSnapshot = captureEnv(["ICLAW_CONFIG_PATH", "ICLAW_TEST_FAST"]);
  try {
    fs.writeFileSync(
      configPath,
      `${JSON.stringify(
...
describe("config cli integration", () => {
  it("supports batch-file dry-run and then writes real config changes", async () => {
    ...
    const configPath = path.join(tempDir, "openclaw.json");
    const batchPath = path.join(tempDir, "batch.json");
    const envSnapshot = captureEnv(["ICLAW_CONFIG_PATH", "ICLAW_TEST_FAST", "DISCORD_BOT_TOKEN"]);
    try {
      fs.writeFileSync(
        configPath,
        `${JSON.stringify(
  ...
  it("keeps file unchanged when real-file dry-run fails and reports JSON error payload", async () => {
    ...
    const configPath = path.join(tempDir, "openclaw.json");
    const envSnapshot = captureEnv(["ICLAW_CONFIG_PATH", "ICLAW_TEST_FAST", "MISSING_TEST_SECRET"]);
    try {
      fs.writeFileSync(
        configPath,
        `${JSON.stringify(
