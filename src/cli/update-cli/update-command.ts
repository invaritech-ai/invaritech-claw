...
const POST_CORE_UPDATE_ENV = "ICLAW_UPDATE_POST_CORE";
const POST_CORE_UPDATE_CHANNEL_ENV = "ICLAW_UPDATE_POST_CORE_CHANNEL";
const POST_CORE_UPDATE_RESULT_PATH_ENV = "ICLAW_UPDATE_POST_CORE_RESULT_PATH";
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  const devTargetRef =
    channel === "dev" ? process.env.ICLAW_UPDATE_DEV_TARGET_REF?.trim() || undefined : undefined;
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating iclaw..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this iclaw install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("openclaw doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("openclaw gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
