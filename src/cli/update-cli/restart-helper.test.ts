...
`,
  ...
  describe("prepareRestartScript", () => {
    ...
    it("uses ICLAW_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        ICLAW_PROFILE: "default",
        ICLAW_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("uses ICLAW_STATE_DIR for the macOS update restart log", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        ICLAW_PROFILE: "default",
        HOME: "/Users/testuser",
        ICLAW_STATE_DIR: "/tmp/openclaw-state",
      });
...
`,
    ...
    it("logs custom macOS launchd labels without shell expansion", async () => {
      ...
      const { scriptPath } = await prepareAndReadScript({
        ICLAW_LAUNCHD_LABEL: "ai.openclaw.$(echo injected)",
        HOME: path.join(tmpDir, "home"),
        ICLAW_STATE_DIR: stateDir,
      });
    ...
    it("uses ICLAW_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        ICLAW_PROFILE: "default",
        ICLAW_LAUNCHD_LABEL: "com.custom.openclaw",
      });
    ...
    it("uses ICLAW_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        ICLAW_PROFILE: "default",
        ICLAW_WINDOWS_TASK_NAME: "OpenClaw Gateway (custom)",
      });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        ICLAW_LAUNCHD_LABEL: "ai.openclaw.it's-a-test",
      });
