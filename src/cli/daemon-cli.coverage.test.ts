...
const buildGatewayInstallPlan = vi.fn(
  async (params: {
    ...
    environment: {
      ICLAW_GATEWAY_PORT: String(params.port),
      ...(params.token ? { ICLAW_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.ICLAW_STATE_DIR = "/tmp/openclaw-cli-state";
    process.env.ICLAW_CONFIG_PATH = "/tmp/openclaw-cli-state/openclaw.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ICLAW_PROFILE: "dev",
        ICLAW_STATE_DIR: "/tmp/openclaw-daemon-state",
        ICLAW_CONFIG_PATH: "/tmp/openclaw-daemon-state/openclaw.json",
