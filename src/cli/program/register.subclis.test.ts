...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.ICLAW_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.ICLAW_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.ICLAW_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.ICLAW_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
    process.env.ICLAW_ENABLE_PRIVATE_QA_CLI = "1";
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      delete process.env.ICLAW_ENABLE_PRIVATE_QA_CLI;
    } else {
      process.env.ICLAW_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "iclaw"]);
