...
describe("private-qa-cli", () => {
  ...
  const originalPrivateQaCli = process.env.ICLAW_ENABLE_PRIVATE_QA_CLI;
  ...
  afterEach(() => {
    ...
    if (originalPrivateQaCli === undefined) {
      ...
      process.env.ICLAW_ENABLE_PRIVATE_QA_CLI = originalPrivateQaCli;
    }
  });
  ...
  it("rejects non-source package roots even when private QA is enabled", async () => {
    ...
    ).toThrow("Private QA CLI is only available from an iclaw source checkout.");
  ...
  it("rejects when the private QA env flag is disabled", async () => {
    ...
    expect(() => loadPrivateQaCliModule({ importModule })).toThrow(
      "Private QA CLI is only available from an iclaw source checkout.",
    );
    expect(importModule).not.toHaveBeenCalled();
  });
});
