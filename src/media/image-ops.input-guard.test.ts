...
describe("image input pixel guard", () => {
  ...
  it("fails closed when sips cannot determine image dimensions", async () => {
    const previousBackend = process.env.ICLAW_IMAGE_BACKEND;
    process.env.ICLAW_IMAGE_BACKEND = "sips";
    try {
      await expect(
        resizeToJpeg({
      ...
      if (previousBackend === undefined) {
        delete process.env.ICLAW_IMAGE_BACKEND;
      } else {
        process.env.ICLAW_IMAGE_BACKEND = previousBackend;
      }
    }
  });
});
