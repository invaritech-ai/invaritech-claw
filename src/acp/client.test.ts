...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets ICLAW_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "openclaw",
    });

    expect(env.ICLAW_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("openclaw");
  });

  it("overrides pre-existing ICLAW_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      ICLAW_SHELL: "wrong",
    });
    expect(env.ICLAW_SHELL).toBe("acp-client");
  ...
  it("preserves ICLAW_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        ICLAW_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["ICLAW_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default OpenClaw bridge", () => {
    ...
    expect(env.ICLAW_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.ICLAW_SHELL).toBe("acp-client");
  });
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("ICLAW_API_KEY")).toBe(false);
  });
});
