...
const ICLAW_CLI_WRAPPER_PATH = fileURLToPath(new URL("../../iclaw.mjs", import.meta.url));
const ICLAW_RUN_NODE_SCRIPT_PATH = fileURLToPath(
  new URL("../../scripts/run-node.mjs", import.meta.url),
);
const ICLAW_DIST_ENTRY_JS_PATH = fileURLToPath(new URL("../../dist/entry.js", import.meta.url));
const ICLAW_DIST_ENTRY_MJS_PATH = fileURLToPath(new URL("../../dist/entry.mjs", import.meta.url));
...
export function resolveLocalAuthCliInvocation(params?: {
  ...
  const hasDistEntry =
    params?.hasDistEntry ??
    (existsSync(ICLAW_DIST_ENTRY_JS_PATH) || existsSync(ICLAW_DIST_ENTRY_MJS_PATH));
  const hasRunNodeScript = params?.hasRunNodeScript ?? existsSync(ICLAW_RUN_NODE_SCRIPT_PATH);
  const command = params?.execPath ?? process.execPath;
  const wrapperPath = params?.wrapperPath ?? ICLAW_CLI_WRAPPER_PATH;
  const runNodePath = params?.runNodePath ?? ICLAW_RUN_NODE_SCRIPT_PATH;
...
export function resolveLocalAuthSpawnCwd(params: { args: string[]; defaultCwd?: string }): string {
  ...
  if (entryBase === "iclaw.mjs" || entryBase === "openclaw.mjs") {
