...
describe("light background detection", () => {
  ...
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      ICLAW_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when ICLAW_THEME=light", async () => {
    const mod = await importThemeWithEnv({ ICLAW_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when ICLAW_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ ICLAW_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats ICLAW_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ ICLAW_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("ICLAW_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      ICLAW_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ ICLAW_THEME: "light" });
    const darkMod = await importThemeWithEnv({ ICLAW_THEME: "dark" });
