...
const IclawSecretRefSchema = z.object({ env: z.string().min(1) }).strict();
const IclawFileSecretRefSchema = z.object({ file: z.string().min(1) }).strict();
const IclawSecretInputSchema = z.union([IclawSecretRefSchema, IclawFileSecretRefSchema]);

const IclawAgentConfigSchema = z
  .object({
...
const IclawProvidersSchema = z
  .object({
    openrouter: z.object({ apiKey: IclawSecretInputSchema }).strict(),
    ollama: z.object({ baseUrl: z.string().url() }).strict(),
  })
  .strict();

const IclawApiConfigSchema = z
  .object({
    baseUrl: z.string().url(),
    auth: z
      .object({
        bearer: IclawSecretInputSchema.optional(),
      })
      .strict()
      .optional(),
    allow: z
...
const IclawWebhookConfigSchema = z
  .object({
    path: z.string().min(1),
    agentId: z.string().min(1),
    secret: IclawSecretInputSchema.optional(),
    idempotency: z
      .object({
...
const IclawScheduleConfigSchema = z
  .object({
    agentId: z.string().min(1),
    schedule: z
      .object({
...
const IclawServerConfigSchema = z
  .object({
    host: z.string().min(1),
    port: z.number().int().min(1).max(65535),
    token: IclawSecretInputSchema.optional(),
  })
  .strict();

const IclawStorageConfigSchema = z
  .object({
...
export const IclawConfigSchema = z
  .object({
    agents: z
      .record(z.string().min(1), IclawAgentConfigSchema)
      .refine((agents) => Object.keys(agents).length > 0, {
        message: "At least one agent is required.",
      }),
    providers: IclawProvidersSchema,
    apis: z.record(z.string().min(1), IclawApiConfigSchema).optional(),
    webhooks: z.record(z.string().min(1), IclawWebhookConfigSchema).optional(),
    schedules: z.record(z.string().min(1), IclawScheduleConfigSchema).optional(),
    server: IclawServerConfigSchema.optional(),
    storage: IclawStorageConfigSchema.optional(),
  })
  .strict();
