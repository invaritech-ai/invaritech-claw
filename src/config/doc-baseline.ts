...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.ICLAW_CONFIG_DOC_BASELINE_DEBUG === "1") {
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    ICLAW_STATE_DIR: path.join(os.tmpdir(), "openclaw-config-doc-baseline-state"),
    ICLAW_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
