...
describe("sessions", () => {
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ ICLAW_STATE_DIR: stateDir }, fn);
  ...
  it("derives session transcripts dir from ICLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ICLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when ICLAW_STATE_DIR differs from stored paths", () => {
