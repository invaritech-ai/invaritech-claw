...
describe("config io paths", () => {
  ...
  it("uses ICLAW_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { ICLAW_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
  ...
  it("honors explicit ICLAW_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".openclaw", 20002, "custom.json");
      const io = createIoForHome(home, { ICLAW_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
