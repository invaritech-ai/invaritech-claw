...
describe("config env vars", () => {
  ...
  it("loads ${VAR} substitutions from ~/.openclaw/.env on repeated runtime loads", async () => {
    await withTempHome(async (_home) => {
      await withEnvOverride({ BRAVE_API_KEY: undefined }, async () => {
        ...
        if (!stateDir) {
          throw new Error("Expected ICLAW_STATE_DIR to be set by withTempHome");
        }
  ...
  it("respects ICLAW_STATE_DIR when reading state-dir .env vars", async () => {
    await withTempHome(async (_home) => {
      const customStateDir = path.join(process.env.ICLAW_STATE_DIR ?? "", "custom-state");
      await writeStateDirDotEnv("CUSTOM_KEY=from-override\n", {
        stateDir: customStateDir,
      });
      expect(
        readStateDirDotEnvVars({
          ICLAW_STATE_DIR: customStateDir,
        }).CUSTOM_KEY,
      ).toBe("from-override");
    });
  });
