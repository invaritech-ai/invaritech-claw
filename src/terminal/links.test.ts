...
import { resolveDocsSiteRoot } from "../infra/product-branding.js";
...
describe("formatDocsLink", () => {
  it("prepends the docs root when given a relative path", () => {
    ...
    expect(out).toContain(`${resolveDocsSiteRoot()}/channels/quietchat`);
  });
  ...
  it("treats whitespace-only path like an empty path and falls back to docs root", () => {
    ...
    expect(out).toContain(resolveDocsSiteRoot());
  });
