...
function writeBundledPlugin(params: {
  ...
  process.env.ICLAW_BUNDLED_PLUGINS_DIR = bundledDir;
...
function withStateDir<T>(run: (stateDir: string) => T) {
  ...
  return withEnv({ ICLAW_STATE_DIR: stateDir }, () => run(stateDir));
}
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.ICLAW_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.ICLAW_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}
...
function createEnvResolvedPluginFixture(pluginId: string) {
  ...
  const env = {
    ...process.env,
    ICLAW_HOME: openclawHome,
    HOME: ignoredHome,
    ICLAW_STATE_DIR: stateDir,
    ICLAW_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
  };
  return { plugin, env };
}
...
`,
  ...
  it("loads bundled runtime deps from an external stage dir", () => {
    ...
    process.env.ICLAW_BUNDLED_PLUGINS_DIR = bundledDir;
    process.env.ICLAW_PLUGIN_STAGE_DIR = stageDir;
    fs.writeFileSync(
      path.join(plugin.dir, "package.json"),
      JSON.stringify(
        {
...
module.exports = { id: "throws-after-import", register() {} };`,
        ...
        return {
          ...
          loadFirst: () =>
            loadOpenClawPlugins({
              ...
              env: {
                ...process.env,
                ICLAW_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadOpenClawPlugins({
              ...
              env: {
                ...process.env,
                ICLAW_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadOpenClawPlugins({
              ...
              env: {
                ...process.env,
                HOME: homeA,
                ICLAW_HOME: undefined,
                ICLAW_STATE_DIR: stateDir,
                ICLAW_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadOpenClawPlugins({
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadOpenClawPlugins({
      env: {
        ...
        HOME: homeDir,
        ICLAW_HOME: undefined,
        ICLAW_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers ICLAW_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const openclawHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
  ...
  it("can include plugin export shape when register is missing", () => {
    ...
    const registry = withEnv({ ICLAW_PLUGIN_LOAD_DEBUG: "1" }, () =>
      loadRegistryFromSinglePlugin({
        plugin,
        pluginConfig: {
...
module.exports = {
  ...
  it("evaluates load-path provenance warnings", () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          const stateDir = makeTempDir();
          return withEnv({ ICLAW_STATE_DIR: stateDir }, () => {
            const globalDir = path.join(stateDir, "extensions", "rogue");
            mkdirSafe(globalDir);
            writePlugin({
              id: "rogue",
  ...
  it("uses the source runtime snapshot allowlist for plugin trust checks", () => {
    useNoBundledPlugins();
    const stateDir = makeTempDir();
    withEnv({ ICLAW_STATE_DIR: stateDir }, () => {
  ...
  if (!Object.getOwnPropertyDescriptor(runtime, "channel")) {
    ...
    const registry = withEnv({ ICLAW_STATE_DIR: stateDir }, () =>
      loadRegistryFromSinglePlugin({
        plugin,
        pluginConfig: {
  ...
  configSchema: (require("openclaw/plugin-sdk").emptyPluginConfigSchema)(),
    ...
    const registry = withEnv({ ICLAW_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins" }, () =>
      loadOpenClawPlugins({
        cache: false,
        workspaceDir: plugin.dir,
        config: {
