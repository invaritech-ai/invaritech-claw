...
describe("startLazyPluginServiceModule", () => {
  afterEach(() => {
    delete process.env.ICLAW_LAZY_SERVICE_SKIP;
    delete process.env.ICLAW_LAZY_SERVICE_OVERRIDE;
  });
  ...
  it("uses the override module when configured", async () => {
    process.env.ICLAW_LAZY_SERVICE_OVERRIDE = "virtual:service";
    const start = createAsyncHookMock();
    const loadOverrideModule = vi.fn(async () => ({ startOverride: start }));

    await expectLifecycleStarted({
      overrideEnvVar: "ICLAW_LAZY_SERVICE_OVERRIDE",
  ...
  it("surfaces override validation failures", async () => {
    process.env.ICLAW_LAZY_SERVICE_OVERRIDE = "data:text/javascript,boom";
