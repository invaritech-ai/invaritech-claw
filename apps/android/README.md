...
7) Canvas host is enabled and reachable from the device (do not run gateway with `ICLAW_SKIP_CANVAS_HOST=1`; startup logs should include `canvas host mounted at .../__openclaw__/`).
...
- `ICLAW_ANDROID_GATEWAY_URL=ws://...` (default: from your local OpenClaw config)
- `ICLAW_ANDROID_GATEWAY_TOKEN=...`
- `ICLAW_ANDROID_GATEWAY_PASSWORD=...`
