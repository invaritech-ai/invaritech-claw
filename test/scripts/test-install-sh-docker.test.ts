...
describe("test-install-sh-docker", () => {
  ...
  it("uses npm latest as the update baseline and resolves it to the concrete packed version", () => {
    ...
    expect(runner).toContain('UPDATE_BASELINE_VERSION="${ICLAW_INSTALL_UPDATE_BASELINE:-latest}"');
    ...
    expect(workflow).toContain("ICLAW_INSTALL_SMOKE_UPDATE_BASELINE: latest");
  });

  it("can reuse dist from the already-built root Docker smoke image", () => {
    const script = readFileSync(SCRIPT_PATH, "utf8");

    expect(script).toContain('UPDATE_DIST_IMAGE="${ICLAW_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"');
...
describe("install-sh smoke runner", () => {
  it("wraps long npm/update operations with heartbeat and install-size audits", () => {
    ...
    expect(script).toContain('HEARTBEAT_INTERVAL="${ICLAW_INSTALL_SMOKE_HEARTBEAT_INTERVAL:-60}"');
    expect(script).toContain(
  ...
  it("covers plain npm global installs and npm-driven updates", () => {
    ...
    expect(script).toContain('SKIP_NPM_GLOBAL="${ICLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"');
    expect(script).toContain('NPM_CACHE_DIR="${ICLAW_INSTALL_SMOKE_NPM_CACHE_DIR:-}"');
    ...
    expect(script).toContain("ICLAW_INSTALL_SMOKE_MODE=npm-global");
...
describe("bun global install smoke", () => {
  it("packs the current tree and verifies image-provider discovery through Bun", () => {
    ...
    expect(script).toContain("ICLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE");
  });
  ...
  it("gates workflow Bun install smoke to scheduled and release-check runs", () => {
    ...
    expect(workflow).toContain(
      "ICLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE: openclaw-dockerfile-smoke:local",
    );
    ...
    expect(workflow).toContain("ICLAW_CI_WORKFLOW_BUN_GLOBAL_INSTALL_SMOKE");
    ...
    expect(workflow).toContain('ICLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL: "1"');
