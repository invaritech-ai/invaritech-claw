...
describe("parallels npm update smoke", () => {
  ...
  it("scrubs future plugin entries before invoking old same-guest updaters", () => {
    ...
    expect(script).toContain("$env:ICLAW_DISABLE_BUNDLED_PLUGINS = '1'");
    expect(script).toContain("ICLAW_DISABLE_BUNDLED_PLUGINS=1 /opt/homebrew/bin/openclaw update");
    expect(script).toContain("ICLAW_DISABLE_BUNDLED_PLUGINS=1 openclaw update");
  });
});
