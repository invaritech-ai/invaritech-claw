...
export function loadVitestExperimentalConfig(
  ...
  if (!windowsEnv && !isDisabled(env.ICLAW_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (windowsEnv && isEnabled(env.ICLAW_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (experimental.fsModuleCache && env.ICLAW_VITEST_FS_MODULE_CACHE_PATH?.trim()) {
    experimental.fsModuleCachePath = env.ICLAW_VITEST_FS_MODULE_CACHE_PATH.trim();
  }
  if (isEnabled(env.ICLAW_VITEST_IMPORT_DURATIONS)) {
    experimental.importDurations = { print: true };
  }
  if (isEnabled(env.ICLAW_VITEST_PRINT_IMPORT_BREAKDOWN)) {
