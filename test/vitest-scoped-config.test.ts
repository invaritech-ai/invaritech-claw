...
describe("resolveVitestIsolation", () => {
  ...
  it("ignores the legacy isolation escape hatches", () => {
    expect(resolveVitestIsolation({ ICLAW_TEST_ISOLATE: "1" })).toBe(false);
    expect(resolveVitestIsolation({ ICLAW_TEST_NO_ISOLATE: "0" })).toBe(false);
    expect(resolveVitestIsolation({ ICLAW_TEST_NO_ISOLATE: "false" })).toBe(false);
  });
...
describe("createScopedVitestConfig", () => {
  ...
  it("loads scoped include overrides from ICLAW_VITEST_INCLUDE_FILE", () => {
...
describe("scoped vitest configs", () => {
  ...
  it("loads channel include overrides from ICLAW_VITEST_INCLUDE_FILE", () => {
